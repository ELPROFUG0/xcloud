    ...
    const animate = (currentTime: number) => {
      if (active && currentTime - lastUpdateRef.current > updateRate) {
        ...
        if (analyserRef.current) {
          ...
          const usefulEnd = Math.floor(dataArray.length * 0.5)
          let sum = 0
          let peak = 0
          for (let i = 0; i < usefulEnd; i++) {
            sum += dataArray[i]
            if (dataArray[i] > peak) peak = dataArray[i]
          }
          const average = sum / usefulEnd / 255
          const peakNorm = peak / 255
          const mixed = (average * 0.4 + peakNorm * 0.6) * sensitivity

          historyRef.current.push(Math.min(1, Math.max(0.06, mixed)))
      ...
      // Smooth sub-pixel scroll between samples
      if (active && lastSampleTime > 0) {
        const elapsed = currentTime - lastSampleTime
        const step = barWidth + barGap
        smoothOffset = (elapsed / updateRate) * step
      }
      ...
      const step = barWidth + barGap
      const barCount = Math.floor(rect.width / step) + 2
      const centerY = rect.height / 2
      ...
      if (dataToRender.length > 0) {
        ...
        const pixelShift = active ? smoothOffset : 0
        ...
        for (let i = 0; i < barCount; i++) {
          ...
          if (dataIndex >= 0 && dataIndex < dataToRender.length) {
            ...
            if (value !== undefined) {
              const x = rect.width - (i + 1) * step - pixelShift
              if (x < -step || x > rect.width + step) continue
              const barHeight = Math.max(
                baseBarHeight,
                value * rect.height * 0.95
              )
              const y = centerY - barHeight / 2
      ...
      if (fadeEdges && fadeWidth > 0) {
        ...
        // Left fade
        const leftW = Math.min(rect.width * 0.18, fadeWidth * 1.5)
        if (leftW > 0) {
          const lg = ctx.createLinearGradient(0, 0, leftW, 0)
          lg.addColorStop(0, "rgba(255,255,255,1)")
          lg.addColorStop(1, "rgba(255,255,255,0)")
          ctx.fillStyle = lg
          ctx.fillRect(0, 0, leftW, rect.height)
        }

        // Right fade
        const rightW = Math.min(rect.width * 0.35, fadeWidth * 3)
        if (rightW > 0) {
          const rg = ctx.createLinearGradient(rect.width - rightW, 0, rect.width, 0)
          rg.addColorStop(0, "rgba(255,255,255,0)")
          rg.addColorStop(1, "rgba(255,255,255,1)")
          ctx.fillStyle = rg
          ctx.fillRect(rect.width - rightW, 0, rightW, rect.height)
        }
