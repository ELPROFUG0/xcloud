...
interface TokenizedCode { tokens: ThemedToken[][]; fg: string; bg: string; }

interface CodeBlockContextType { code: string; }
...
const getHighlighter = (language: BundledLanguage): Promise<HighlighterGeneric<BundledLanguage, BundledTheme>> => {
  ...
  const promise = createHighlighter({ langs: [language], themes: ["github-dark"] });
...
export const highlightCode = (
  ...
  getHighlighter(language)
    .then((highlighter) => {
      ...
      const lang = available.includes(language) ? language : "text";
      const result = highlighter.codeToTokens(code, { lang, themes: { dark: "github-dark", light: "github-dark" } });
      const tokenized: TokenizedCode = { bg: result.bg ?? "transparent", fg: result.fg ?? "inherit", tokens: result.tokens };
      tokensCache.set(key, tokenized);
      const subs = subscribers.get(key);
      if (subs) { for (const sub of subs) sub(tokenized); subscribers.delete(key); }
    })
  ...
  ({ tokenized, showLineNumbers, className }: { tokenized: TokenizedCode; showLineNumbers: boolean; className?: string }) => {
    ...
    return (
      <pre className={cn("m-0 p-4 text-sm", className)} style={preStyle}>
        <code className={cn("font-mono text-[12px] leading-[1.7]", showLineNumbers && "[counter-increment:line_0] [counter-reset:line]")}>
          {keyedLines.map((kl) => <LineSpan key={kl.key} keyedLine={kl} showLineNumbers={showLineNumbers} />)}
        </code>
      </pre>
    );
  },
  (prev, next) => prev.tokenized === next.tokenized && prev.showLineNumbers === next.showLineNumbers && prev.className === next.className
);
CodeBlockBody.displayName = "CodeBlockBody";

export const CodeBlockContainer = ({ className, language, style, ...props }: HTMLAttributes<HTMLDivElement> & { language: string }) => (
  <div
    className={cn("group/cb relative w-full overflow-hidden rounded-lg bg-[#1D1D1D]", className)}
...
export const CodeBlockHeader = ({ children, className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("flex items-center justify-between bg-[#252525] px-3 py-1.5 text-text-muted/60 text-[11px]", className)} {...props}>
    {children}
  </div>
);
...
export const CodeBlockFilename = ({ children, className, ...props }: HTMLAttributes<HTMLSpanElement>) => (
  <span className={cn("font-mono", className)} {...props}>{children}</span>
);

export const CodeBlockActions = ({ children, className, ...props }: HTMLAttributes<HTMLDivElement>) => (
  <div className={cn("-my-1 -mr-1 flex items-center gap-2 opacity-0 group-hover/cb:opacity-100 transition-opacity", className)} {...props}>{children}</div>
);
...
export const CodeBlockContent = ({ code, language, showLineNumbers = false }: { code: string; language: BundledLanguage; showLineNumbers?: boolean }) => {
  ...
  useEffect(() => {
    let cancelled = false;
    highlightCode(code, language, (result) => { if (!cancelled) setAsyncTokens(result); });
    return () => { cancelled = true; };
  }, [code, language]);

  return (
    <div className="relative overflow-auto">
      <CodeBlockBody showLineNumbers={showLineNumbers} tokenized={asyncTokens ?? syncTokens} />
    </div>
  );
};
...
export const CodeBlockCopyButton = ({ onCopy, onError, timeout = 2000, children, className, ...props }: {
  onCopy?: () => void; onError?: (error: Error) => void; timeout?: number;
