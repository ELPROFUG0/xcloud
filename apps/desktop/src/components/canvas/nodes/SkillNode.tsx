...
export function SkillNode({ data }: NodeProps) {
  ...
  return (
    <div className="rounded-xl border border-amber-800/60 bg-amber-950/30 px-4 py-3 min-w-[140px]">
      ...
      {d.skills.length > 0 && (
        <div className="mt-1.5 space-y-0.5">
          {d.skills.slice(0, 3).map((s) => (
            <div key={s} className="text-[9px] text-amber-300/50 truncate">{s}</div>
          ))}
          {d.skills.length > 3 && (
            <div className="text-[9px] text-amber-300/40">+{d.skills.length - 3} more</div>
          )}
        </div>
      )}
    </div>
